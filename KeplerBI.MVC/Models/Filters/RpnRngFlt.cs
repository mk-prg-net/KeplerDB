...
namespace KeplerBI.MVC.Models.Filters
{
    ...
    {
        public RpnRngFlt(string ControllerName, Parser.RPN.IFunctionNames fn, IToken[] tokens, IToken[] fSubtree, params string[] ParameterNames) 
            : base(ControllerName, fn, tokens, fSubtree, ParameterNames)
        {
            ...
            BeginName = fn.IsSemanticDescriptor(Key) ? fn.ReduceSematicFunctionName(Key) : Key;
            Begin = ParameterSubTrees[0].Value.ToPNString();

            Key = ParameterSubTrees[1].Key;
            EndName = fn.IsSemanticDescriptor(Key) ? fn.ReduceSematicFunctionName(Key) : Key;
